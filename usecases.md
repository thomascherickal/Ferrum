# Ten Use Cases for Ferrum

Ferrum is a zero-dependency, CPU-only, pure-Rust engine for small Transformers,
SLMs, and MLPs. Its sweet spot is anywhere a small, self-contained, predictable
model has to run without a GPU, a Python runtime, or a network connection. Here
are ten scenarios where that combination is a genuine advantage.
...
tokenizer means the same model handles any UTF-8 input — including accented and
non-Latin scripts — without an unknown-token escape hatch.
...
## 2. In-browser AI playgrounds (WebAssembly)
...
Train a small subword model on logs, command histories, code snippets, or
chat templates to power next-token suggestions inside a CLI, an editor plugin,
or an embedded form. A BPE vocabulary captures recurring multi-character motifs
(flags, identifiers, file paths) far more compactly than a character model.
...
## 5. Reproducible research and teaching

Every layer, the optimizer, the tokenizer, and the file format are written in
readable Rust with no hidden CUDA kernels or opaque dependencies. Students and
researchers can read the entire forward and backward pass, set a seed, and get
bit-for-bit reproducible results — a transparent reference implementation of
attention, BPE, and quantization-aware training.

**Why Ferrum:** the whole stack is inspectable, deterministic, and dependency-free.

---

## 6. Tabular classification and regression at the edge
...
## 7. Embedded and resource-constrained systems

With no allocator surprises from heavy dependencies and int8 models a few tens
of kilobytes in size, Ferrum fits microcontroller-class budgets and firmware
images. The CPU-only design means no driver stack and predictable, single-
threaded execution.
...
## 8. Air-gapped and field deployments
...
## 9. Cost-free, scalable inference services
...
classifications at high throughput with no model server, no GPU scheduler, and
no inference framework to operate.
...
## 10. Rapid prototyping of model ideas
...
BPE tokenization — on the same corpus in minutes, all with identical APIs.
...
|----------------------------------------|-------------------------------------------------------------|
| Smallest possible model                | `train_embedded`, BPE vocab 256–512, int8 save               |
...
See [usecases](usecases.md) in context with the worked [examples](example.md)
and the [evaluation guide](evaluation.md) to measure quality, size, and speed.
