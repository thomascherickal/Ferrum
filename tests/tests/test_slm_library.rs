...
fn test_generate_continuation_excludes_seed_and_matches_generate() {
    ...
    let cont = slm.generate_continuation(seed, 12, 0.1, &mut Rng::new(99)).unwrap();

    assert!(!cont.starts_with(seed), "continuation should not repeat the seed");
    assert_eq!(cont.chars().count(), 12, "continuation must honour the char budget");
    assert_eq!(format!("{seed}{cont}"), full, "seed + continuation must equal generate()");
}
...
fn test_generate_continuation_bpe_path() {
    ...
    let cont = slm.generate_continuation(seed, 10, 0.4, &mut Rng::new(5)).unwrap();
...
fn test_generate_stream_concatenates_to_continuation_char_level() {
    ...
    assert_eq!(format!("{seed}{streamed}"), full, "fragments ≠ continuation");
...
fn test_generate_stream_matches_generate_bpe() {
    ...
    let mut streamed = String::new();
    let full = slm
        .generate_stream(seed, 24, 0.5, &mut Rng::new(9), |frag| streamed.push_str(frag))
        .unwrap();

    let reference = slm.generate(seed, 24, 0.5, &mut Rng::new(9)).unwrap();
    assert_eq!(full, reference, "stream return differs from generate() (BPE)");
    ...
    assert!(!streamed.contains('\u{FFFD}'), "streamed a U+FFFD placeholder");
}
...
fn test_generate_stream_handles_non_ascii_without_replacement_chars() {
    ...
    let mut streamed = String::new();
    let full = slm
        .generate_stream(seed, 20, 0.6, &mut Rng::new(2), |frag| streamed.push_str(frag))
        .unwrap();
    let continuation: String = full.chars().skip(seed.chars().count()).collect();
...
fn test_threaded_training_single_thread_matches_serial_end_to_end() {
    ...
    let serial = GenerativeSLM::train_transformer(
        corpus, 8, 16, 2, 1, 32, 40, 0.01, 8, 0, &mut Rng::new(7),
    ).unwrap();
    let threaded = GenerativeSLM::train_transformer_threaded_with_callback(
        corpus, 8, 16, 2, 1, 32, 40, 0.01, 8, 0, 1, &mut Rng::new(7), |_, _| {},
    ).unwrap();
...
fn test_threaded_training_multi_thread_learns() {
    ...
    let slm = GenerativeSLM::train_transformer_threaded_with_callback(
        corpus, 4, 16, 2, 1, 32, 60, 0.01, 16, 0, 4, &mut Rng::new(7),
        |_, loss| losses.push(loss),
    ).unwrap();

    assert!(
        losses.last().unwrap() < &(losses[0] * 0.5),
        "threaded loss did not halve: {} → {}", losses[0], losses.last().unwrap()
    );
...
fn test_threaded_training_reproducible_across_runs() {
    ...
    let train = || {
        GenerativeSLM::train_transformer_threaded_with_callback(
            corpus, 6, 16, 2, 1, 32, 30, 0.01, 8, 300, 4, &mut Rng::new(7), |_, _| {},
        )
        .unwrap()
        .to_bytes()
        .unwrap()
    };
    assert_eq!(train(), train(), "threaded training is not reproducible run-to-run");
}
...
fn test_evaluate_transformer_memorizes_training_corpus() {
    ...
    let mut rng = Rng::new(7);
    let slm = GenerativeSLM::train_transformer(
        corpus, 4, 16, 2, 1, 32, 120, 0.01, 8, 0, &mut rng,
    ).unwrap();

    let eval = slm.evaluate(corpus).unwrap();
    assert_eq!(eval.num_predictions, corpus.chars().count() - 4);
    assert!(eval.perplexity >= 1.0, "perplexity cannot be below 1.0: {}", eval.perplexity);
    assert!(eval.perplexity < 1.5, "memorized perplexity too high: {}", eval.perplexity);
    assert!(eval.cross_entropy < 0.4, "memorized cross-entropy too high: {}", eval.cross_entropy);
...
fn test_evaluate_perplexity_beats_uniform_baseline() {
    ...
    let slm = GenerativeSLM::train_transformer(
        corpus, 8, 24, 4, 2, 48, 80, 0.01, 8, 0, &mut rng,
    ).unwrap();
...
fn test_evaluate_works_for_all_three_paths_and_bpe() {
    ...
    let embedded = GenerativeSLM::train_embedded(corpus, 6, 16, 48, 40, 0.05, 0.9, 8, 0, &mut rng).unwrap();
    let transformer = GenerativeSLM::train_transformer(corpus, 6, 16, 2, 1, 32, 40, 0.01, 8, 0, &mut rng).unwrap();
    let bpe = GenerativeSLM::train_transformer(corpus, 6, 16, 2, 1, 32, 40, 0.01, 8, 300, &mut rng).unwrap();
    ...
    for (name, slm) in [
        ...
        assert!(eval.perplexity >= 1.0, "{name}: perplexity {} below 1.0", eval.perplexity);
...
fn test_evaluate_survives_finf_roundtrip() {
    ...
    let mut rng = Rng::new(7);
    let slm = GenerativeSLM::train_transformer(
        corpus, 4, 16, 2, 1, 32, 60, 0.01, 8, 0, &mut rng,
    ).unwrap();
...
fn test_evaluate_rejects_text_shorter_than_context() {
    ...
    let mut rng = Rng::new(7);
    let slm = GenerativeSLM::train_transformer(
        corpus, 8, 8, 2, 1, 16, 10, 0.01, 8, 0, &mut rng,
    ).unwrap();
